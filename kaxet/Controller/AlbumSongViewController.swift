...
class AlbumSongViewController: UIViewController {
    ...
    private var album: NSDictionary = [:]
    ...
    @IBOutlet weak var coverImageView: KxCustomImageView!
    @IBOutlet weak var albumNameView: UIView!
    ...
    func setHeaderData() {
        let albumphotopath = album["albumphotopath"] as? String
        let albumname = album["albumname"] as? String
        //let artistname = album["artist"] as? String
        let labelname = album["labelname"] as? String
        let albumyear = album["albumyear"] as? String
        let genre = album["genrevalue"] as? String
        /*
        let albumid = album["_id"] as? String
        print(albumid!)
        */
        coverImageView.loadImageUsingUrlString(urlString: albumphotopath)
        coverView.clipsToBounds = true
        albumImage.loadImageUsingUrlString(urlString: albumphotopath)
        albumImage.layer.cornerRadius = 5
        albumImage.clipsToBounds = true
        
        albumNameLabel.text = albumname
        labelNameLabel.text = labelname
        genreNameLabel.text = genre
        albumYearLabel.text = albumyear
        
        albumNameView.layer.cornerRadius = 5
        albumNameView.clipsToBounds = true
        albumNameView.backgroundColor = UIColor(hex: 0x333, alpha: 0.3)
        ...
        apiServices.executePostRequestWithToken(urlToExecute: strUrl, bodyDict: nil) { (jsonResponse, error) in
            ...
            if success! {
                ...
                if let dataResult = responseDict["data"] as? NSArray {
                    ...
                    self.getSongsByAlbum(page: 1, isInit: true)
                    DispatchQueue.main.async {
    ...
    func getSongsByAlbum(page: Int, isInit: Bool) {
        
        //showProgressHud()
        DispatchQueue.global(qos: .userInteractive).async {
            DispatchQueue.main.async {
                let albumid = self.album["_id"] as? String
                
                //Send HTTP request to perform Get Top Songs
                let restUrl = self.apiUrl + "/songln/aggreportln2/\(self.userid)?page=\(page)"
                
                let postString = ["status": "STSACT", "songpublish": "Y", "albumid": albumid!] as NSDictionary
                //let postString = ["status": "STSACT", "songpublish": "Y"] as NSDictionary
    ...
    @IBAction func btnPrevPagePressed(_ sender: UIButton) {
        ...
        self.loadUserPlaylistDone = true
        self.loadSongDone = false
        getSongsByAlbum(page: self.currentPage, isInit: false)
...
extension AlbumSongViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        guard let songtablecell = tableView.dequeueReusableCell(withIdentifier: "SongListCell", for: indexPath) as? SongListTableViewCell else {
        ...
        songtablecell.initData(data: songData)
        
        songtablecell.albumImage.isHidden = true
        let page = songPageControl.currentPage
        let basePageNo = page * 10
        songtablecell.numberSongLabel.text = "\(basePageNo + indexPath.row + 1)"
        
        let pcsFlag = songData["pcsflag"] as? String
        songtablecell.playOrBuyTapAction = {
            if pcsFlag == "Y" {
        ...
        songtablecell.addToPlaylistTapAction = {
            // implement your logic here, e.g. call preformSegue()
            self.addToPlaylist(data: songData)
        }
        
        songtablecell.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(tapTableSongCell(_:))))
        
        return songtablecell
    }
...
extension AlbumSongViewController: BuySongViewControllerDelegate {
